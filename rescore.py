...
def download_multiple_mats(mat_urls, temp_dir, limit=5):
    mat_paths = []
    for url in mat_urls[:limit]:  # ✅ Limit to 5 for performance
...
def process_video_analysis(gaze_data_per_viewer, video_path, alpha=0.007, window_size=20):
    ...
    df['Convex Area (Rolling Avg)'] = df['Convex Area'].rolling(window=window_size, min_periods=1).mean().fillna(0)
    df['Concave Area (Rolling Avg)'] = df['Concave Area'].rolling(window=window_size, min_periods=1).mean().fillna(0)

    # ✅ Correct F-C score formula
...
if st.session_state.data_processed:
    ...
    current_frame = st.session_state.current_frame  # sync ใหม่
