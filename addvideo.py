...
def draw_hulls_on_frame(gaze_points, frame, alpha=0.007):
    if frame is None or len(gaze_points) < 3:
        return
    fig, ax = plt.subplots()
    ax.imshow(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

    pts = np.array(gaze_points)
    try:
        hull = ConvexHull(pts)
        for simplex in hull.simplices:
            ax.plot(pts[simplex, 0], pts[simplex, 1], 'g-', lw=2)
    except:
        pass
